...
/**
 * Blows the site to the default values without modifying any files.
 */
class WP_Blow_CLI extends WP_CLI_Command
{

  /**
   * Blow the site database to default values. No files are modified.
   *
   * ## OPTIONS
   *
   * [--reactivate-theme]
   * : Reactivate currently active theme after blow.
   *
   * [--reactivate-plugins]
   * : Reactivate all currently active plugins after blow.
   *
   * [--reactivate-webhooks]
   * : Reactivate WP Webhooks plugin after blow.
   *
   * [--deactivate-wp-blow]
   * : Deactivate WP Blow plugin after blow. By default it will stay active after blow.
   *
   * [--yes]
   ...
   *
   * $ wp blow blow --yes
   * Success: Database has been blow.
   *
   * @when after_wp_load
   */
  function blow($args, $assoc_args)
  {
    WP_CLI::confirm('Are you sure you want to blow the site? There is NO UNDO!', $assoc_args);
    ...
    if (!empty($assoc_args['disable-wp-blow'])) {
      $params['reactivate_wpblow'] = false;
    } else {
      $params['reactivate_wpblow'] = true;
    }
    ...
    $result = $wp_blow->do_reinstall($params);
    if (is_wp_error($result)) {
      WP_CLI::error($result->get_error_message);
    } else {
      WP_CLI::success('Database has been blow.');
    }
  } // blow


  /**
   * Display WP Blow version.
   *
   * @when after_wp_load
   */
  function version($args, $assoc_args)
  {
    ...
    WP_CLI::line('WP Blow v' . $wp_blow->version);
  } // version
   ...
   * ## EXAMPLES
   *
   * $ wp blow delete themes --yes
   * Success: 3 themes have been deleted.
   *
   * $ wp blow delete custom-tables --truncate --yes
   * Success: 3 custom tables have been emptied.
   *
   * $ wp blow delete htaccess --yes
    ...
    switch ($subcommand) {
      ...
      case 'custom-tables':
        if (!empty($assoc_args['empty'])) {
          ...
          $cnt = $wp_blow->do_truncate_custom_tables();
          ...
          $cnt = $wp_blow->do_drop_custom_tables();
      ...
      case 'htaccess':
        ...
        $tmp = $wp_blow->do_delete_htaccess();
        if (!is_wp_error($tmp)) {
        ...
        break;
      case 'theme-options':
        WP_CLI::confirm('Are you sure you want to blow all options (mods) for all themes?', $assoc_args);
        $cnt = $wp_blow->do_blow_theme_options();
        WP_CLI::success('Options for ' . $cnt . ' themes have been blow.');
        break;
      default:
   ...
   * ## EXAMPLES
   *
   * wp blow snapshots create --yes
   * Success: New snapshot with ID 089bea has been created.
   *
   * $ wp blow snapshots delete --id=123456
   * Success: Snapshot has been deleted.
   *
   * $ wp blow snapshots export --id=123456
   * Success: Snapshot has been exported and saved to: https://test.site/wp-content/wp-blow-snapshots-export/wp-blow-snapshot-123456.sql.gz
   *
   * @when after_wp_load
   */
  function snapshots($args, $assoc_args)
  {
    ...
    switch ($subcommand) {
      case 'list':
        if ($snapshots = $wp_blow->get_snapshots()) {
          $table = array();
          foreach ($snapshots as $ss) {
            ...
            $tmp['info'] .= ' totaling ' . $wp_blow->format_size($ss['tbl_size']) . ' in ' . number_format($ss['tbl_rows']) . ' rows';
      ...
      case 'create':
        ...
        $new = $wp_blow->do_create_snapshot($name);
        if (is_wp_error($new)) {
      ...
      case 'restore':
        if (empty($assoc_args['id'])) {
          WP_CLI::error('Please specify the snapshot ID with the "--id=123456" param. Use "wp blow snapshots list" to get a list of all snapshots.');
          break;
        } else {
          WP_CLI::confirm('Are you sure you want to restore the site to the snapshot with ID ' . $assoc_args['id'] . '?', $assoc_args);
          $restore = $wp_blow->do_restore_snapshot($assoc_args['id']);
      ...
      case 'export':
        if (empty($assoc_args['id'])) {
          ...
          $export = $wp_blow->do_export_snapshot($assoc_args['id']);
          if (is_wp_error($export)) {
            WP_CLI::error($export->get_error_message());
          } else {
            $url = content_url() . '/' . $wp_blow->snapshots_folder . '/' . $export;
      ...
      case 'delete':
        if (empty($assoc_args['id'])) {
          ...
          $del = $wp_blow->do_delete_snapshot($assoc_args['id']);
          if (is_wp_error($del)) {
  ...
  /**
   * This command is no longer available. Please use "wp blow snapshots create" instead.
   */
  function backups($args, $assoc_args)
  {
    WP_CLI::error('This command is no longer available. Please use: wp blow snapshots create');
  } // backups
} // WP_Blow_CLI

WP_CLI::add_command('blow', 'WP_Blow_CLI');
